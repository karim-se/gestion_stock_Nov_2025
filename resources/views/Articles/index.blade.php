                ...
                <td>
                    ...
                    @can('validate_article')
                        @if($article->statut === 'en_attente')
                            <button type="button" onclick="openModal('validerModal{{ $article->id }}')">Valider</button>
                            <button type="button" onclick="openModal('rejeterModal{{ $article->id }}')">Rejeter</button>
                        @endif
                    @endcan
                </td>
            </tr>
...
@foreach($articles as $article)
    @can('validate_article')
        ...
        <div id="validerModal{{ $article->id }}" style="display:none;">
            ...
            <button type="button" onclick="closeModal('validerModal{{ $article->id }}')">Annuler</button>
        ...
        <div id="rejeterModal{{ $article->id }}" style="display:none;">
            ...
            <form action="{{ route('articles.articles-staging.rejeter', $article) }}" method="POST">
                ...
                <button type="button" onclick="closeModal('rejeterModal{{ $article->id }}')">Annuler</button>
